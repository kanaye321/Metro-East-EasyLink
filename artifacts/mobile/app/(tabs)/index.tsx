...
// ── Rider row ────────────────────────────────────────────────────────────────
function RiderRow({ rider }: { rider: any }) {
  const colors = useColors();
  ...
  return (
    <View style={[styles.riderRow, { backgroundColor: colors.card, borderColor: rider.speaking ? colors.primary + '88' : colors.border, borderWidth: rider.speaking ? 1.5 : 1 }]}>
...
export default function HomeScreen() {
  ...
  const { currentUser, rideGroup, connectionType, connectedRiders, serverConnected, createGroup, joinGroup, leaveGroup } = useApp();
        ...
        {connectedRiders.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={styles.riderList}>
              {connectedRiders.map(r => <RiderRow key={r.id} rider={r} />)}
            </View>
          </View>
        )}
