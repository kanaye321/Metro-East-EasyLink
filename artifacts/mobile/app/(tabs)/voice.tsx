...
export default function VoiceScreen() {
    ...
    (async () => {
      try {
        ...
        reader.onload = () => {
          ...
          if (buf && buf.byteLength > 0) socket.emit('voice:audio', buf);
        };
  ...
  useEffect(() => {
    if (!socket) return;
    const onIncomingAudio = (payload: ArrayBuffer) => {
      if (!payload || !(payload as any).byteLength) return;
      ...
      playQueueRef.current.push(payload);
      playNextInQueue();
    };
