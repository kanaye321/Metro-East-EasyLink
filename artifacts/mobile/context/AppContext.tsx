...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (groupStr) { setRideGroup(JSON.parse(groupStr)); }
      } catch {}
      setIsLoading(false);
    })();
  }, []);
  ...
  const createGroup = useCallback(async (name: string) => {
    ...
    if (!socket || !serverConnected) {
      ...
      const group: RideGroup = { id: Date.now().toString(), name, leaderId: currentUser.id, inviteCode: code };
  ...
  const joinGroup = useCallback(async (code: string): Promise<boolean> => {
    ...
    if (!socket || !serverConnected) {
      const group: RideGroup = { id: Date.now().toString(), name: 'Ride Group', leaderId: '', inviteCode: code.toUpperCase() };
      await AsyncStorage.setItem('@rl_group', JSON.stringify(group));
      setRideGroup(group); setConnectedRiders([]);
      return true;
    }
  ...
  return (
    <AppContext.Provider value={{
      ...
      createGroup, joinGroup, leaveGroup, toggleBluetooth, broadcastLocation,
    }}>
      {children}
    </AppContext.Provider>
  );
