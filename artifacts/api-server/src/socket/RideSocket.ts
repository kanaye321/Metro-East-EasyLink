 ...
 * Groups, rider presence, real-time chat, location sharing, and Mumble voice gateway.
 */
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('group:create', ({ name, userId, userProfile }: {
      ...
      try {
        ...
        socket.emit('group:created', { group: { id: group.id, name: group.name, leaderId: group.leaderId, inviteCode: code } });
    ...
    socket.on('group:join', ({ code, userId, userProfile }: {
      ...
      socket.emit('group:joined', {
        group: { id: group.id, name: group.name, leaderId: group.leaderId, inviteCode: upperCode },
        riders: memberList,
      });
    ...
    // ── Chat: send message ─────────────────────────────────────────────────
    socket.on('chat:send', ({ content, type, latitude, longitude }: {
      ...
      io.to(meta.inviteCode).emit('chat:message', { message: msg });
    });
    ...
    socket.on('voice:join', async () => {
      ...
      try {
        ...
        // Relay incoming Mumble audio to the socket
    ...
    socket.on('voice:audio', (audioChunk: Buffer) => {
      const meta = socketMeta.get(socket.id);
      if (!meta) return;
      // Relay raw audio to all OTHER group members (peer relay)
      socket.to(meta.inviteCode).emit('voice:audio', audioChunk);
      // Also forward to Mumble if connected
      const conn = mumbleManager.getConnection(meta.inviteCode, meta.userId);
      if (conn?.connected) conn.sendAudio(audioChunk);
    });
